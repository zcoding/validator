...
规则字符串是用于描述一个规则的表达式，有两种描述形式（带参数或不带参数）
1. `规则名称`
2. `规则名称:参数列表`
...
`checked`规则适用于checkbox或者radio类型的表单

`checked`表示至少选了一个，如果没选就会验证失败
`checked:value1,value2`表示选中了值为value1和值为value2的多选框
`checked:value1`表示选中了值为value1的单选框或者多选框

更复杂的情况请使用集合型规则`include`

### FormValidator中的`selected`规则详解
`selected`规则适用于select类型的表单

`selected:value1`表示选了值为value1的下拉框
`selected:value1,value2`表示选了值为value1和值为value2的下拉框（必须是多选下拉框）
...
{
  if: 'include:[A, B]', // 包含了A,B或其子集（只要包含一个）
  fail: function() {}
}

{
  if: 'include:[(A,B), C]', // 包含了A和B，或者包含了C，两种情况或其子集（只要满足一个）
  fail: function() {}
}

{
  if: 'include:([A,B], C)', // 包含了A,B或其子集（只要包含一个），且包含C
  fail: function() {}
}
```

##匿名规则
匿名规则就是没有定义就直接使用的规则。类似自定义规则，匿名规则可以是一个正则表达式或者函数。区别于非匿名规则，匿名规则没有定义就直接使用。
```javascript
{
  if: /down$/, // 这是一个匿名规则，其规则是：匹配以down结尾的字符串
  fail: function() {}
}
{
  if: function (values) { // 这是一个匿名规则，其规则是：匹配大于10的数字
...
```
匿名规则不是匿名变量！匿名规则是指没有定义的规则，但是匿名规则的变量可以是非匿名的。
```javascript
{
  if: function checkYourSister() {
    // do something...  
  },
  fail: function() {}
}
